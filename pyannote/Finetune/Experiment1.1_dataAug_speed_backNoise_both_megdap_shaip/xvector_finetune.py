...
config_file_path = "/data/Root_content/Vaani/Speaker_ID/pyannote/Fine_tune/Experiment1.1_dataAug_speed_backNoise_both_megdap_shaip/config.yaml"
...
output_model_dir = "/data/Root_content/Vaani/Speaker_ID/pyannote/Fine_tune/Experiment1.1_dataAug_speed_backNoise_both_megdap_shaip/Models"
output_plot_dir = "/data/Root_content/Vaani/Speaker_ID/pyannote/Fine_tune/Experiment1.1_dataAug_speed_backNoise_both_megdap_shaip/Plots"
...
def fine_tune_model(config, model, device, max_train_samples=None, max_val_samples=None):
    try:
        ...
        plots_folder = os.path.join(output_plot_dir, f"Plots_lr={config['optimizer_args']['lr']}_weight_decay={config['optimizer_args']['weight_decay']}_amsgrad={config['optimizer_args']['amsgrad']}")
        models_folder = os.path.join(output_model_dir, f"Models_lr={config['optimizer_args']['lr']}_weight_decay={config['optimizer_args']['weight_decay']}_amsgrad={config['optimizer_args']['amsgrad']}")
        
        # Create directories for storing plots and models
        ...
        for epoch in range(config['num_epochs']):
            print(f"lr={config['optimizer_args']['lr']}_weight_decay={config['optimizer_args']['weight_decay']}_amsgrad={config['optimizer_args']['amsgrad']}")
            # Training
            train_loss = 0.0
            correct_train = 0
            total_train = 0
            ...
            print(f"Epoch {epoch+1}/{config['num_epochs']}, Train Loss: {train_loss:.4f}, Train Accuracy: {train_accuracy:.4f}, EER: {eer:.6f}")

            # Save the model after every epoch
            architecture_name = config['model']
            save_path = os.path.join(models_folder, f"{architecture_name}_epoch{epoch+1}_EER{eer:.6f}.pt")
        ...
        plot_training_results(train_loss_list, train_accuracy_list, eer_list, config['num_epochs'], plots_folder)
...
def main():
    ...
    configurations = [
        {"lr": 0.0001, "weight_decay": 0, "amsgrad": True},
        {"lr": 0.0001, "weight_decay": 0, "amsgrad": False},
        {"lr": 0.0001, "weight_decay": 5e-4, "amsgrad": True},
        {"lr": 0.0001, "weight_decay": 5e-4, "amsgrad": False},
        {"lr": 0.001, "weight_decay": 0, "amsgrad": True},
        {"lr": 0.001, "weight_decay": 5e-4, "amsgrad": True},
        {"lr": 0.001, "weight_decay": 5e-4, "amsgrad": False}
    ]

    for idx, config_mod in enumerate(configurations):
        print(f"\nRunning configuration {idx + 1}/{len(configurations)}")
        
        # Update the optimizer arguments in the YAML config
        config['optimizer_args']['lr'] = config_mod['lr']
        config['optimizer_args']['weight_decay'] = config_mod['weight_decay']
        config['optimizer_args']['amsgrad'] = config_mod['amsgrad']

        # Load the model again (to start fresh for each configuration)
        ...
        # Fine-tune the model with the current configuration
        fine_tune_model(config, model, device, max_train_samples, max_val_samples)
